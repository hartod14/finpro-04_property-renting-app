...
export const PanelPagination: React.FC<props> = (props) => {
  ...
  // Calculate total pages correctly from total items
  const totalItems = safeNumber(total, 0);
  const safeTotalPage = safeNumber(
    totalPage,
  ...
  const generatePages = () => {
    ...
    if (safeTotalPage <= 7) {
      ...
      for (
        ...
        i <= Math.min(safeTotalPage - 1, page + range);
        i++
      ) {
        pages.push(i);
      }
  ...
  // Calculate entry range being displayed
    ...
    <div className="flex justify-between items-center gap-2">
      ...
      <div className="flex gap-1">
        ...
        <button
          ...
          className={`focus:outline-none disabled:cursor-not-allowed w-8 h-8  flex justify-center items-center rounded-lg ${
            page > 1 ? 'bg-blue-900' : 'bg-[#C8CED5]'
          }`}
        >
          <svg
            width="11"
        ...
        {generatePages().map((p, index) => (
          <span
            ...
            className={`cursor-pointer w-8 h-8 flex justify-center items-center font-semibold rounded-lg border-2 ${
              p === page
        ...
        <button
          ...
          onClick={next}
          className={`focus:outline-none w-8 h-8 disabled:cursor-not-allowed flex justify-center items-center rounded-lg ${
            safeTotalPage > 1 && safeTotalPage > page
              ? 'bg-blue-900'
              : 'bg-[#C8CED5]'
          }`}
        >
