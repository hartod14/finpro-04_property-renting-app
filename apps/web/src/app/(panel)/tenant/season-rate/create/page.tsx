...
import TenantSeasonRateCreateModel from '@/models/tenant-panel/tenantSeasonRateCreateModel';
...
export default function TenantSeasonRateCreatePage() {
  const { isLoading, tenantProperty, handleCreateSeasonRate } =
    TenantSeasonRateCreateModel();

  return (
    <div className="bg-white rounded-md p-4 border border-gray-100 shadow-md">
      <Formik<ISeasonRateCreate>
        initialValues={seasonRateInit}
        validationSchema={storeSeasonRateValidator}
        enableReinitialize
        onSubmit={async (values) => {
          await handleCreateSeasonRate(values);
        }}
      >
        {(formik) => {
          return (
