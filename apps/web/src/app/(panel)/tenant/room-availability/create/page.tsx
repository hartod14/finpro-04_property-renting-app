...
import TenantRoomAvailabilityCreateModel from '@/models/tenant-panel/tenantRoomAvailabilityCreateModel';
...
export default function TenantRoomAvailabilityCreatePage() {
  const { isLoading, tenantProperty, handleCreateRoomAvailability } =
    TenantRoomAvailabilityCreateModel();

  return (
    <div className="bg-white rounded-md p-4 border border-gray-100 shadow-md">
      <Formik<IRoomAvailabilityCreate>
        initialValues={roomAvailabilityInit}
        validationSchema={storeRoomAvailabilityValidator}
        enableReinitialize
        onSubmit={async (values) => {
          await handleCreateRoomAvailability(values);
        }}
      >
        {(formik) => {
          return (
                              ...
                              >
                                <label className="flex items-start cursor-pointer">
                                  <input
                                    ...
                                    checked={formik.values.rooms.includes(
                                      room.id,
                                    )}
                                  />
                                  <div className="flex items-center gap-2">
