...
export const tenantTransactionController = {
  async getTenantOrders(req: Request, res: Response) {
    try {
      ...
      const orders = await tenantTransactionService.getTenantOrders(tenantId, status);
      res.json(orders);
    } catch (error: any) {
  ...
  async confirmPayment(req: Request, res: Response) {
    try {
      ...
      const result = await tenantTransactionService.confirmPayment(tenantId, bookingId, accept);
      res.json(result);
    } catch (error: any) {
  ...
  async cancelUserOrder(req: Request, res: Response) {
    try {
      ...
      const result = await tenantTransactionService.cancelUserOrder(tenantId, bookingId);
      res.json(result);
    } catch (error: any) {
      res.status(400).json({ message: error.message });
    }
  ...
  async getPropertyList(req: Request, res: Response) {
    try {
      const tenantId = parseInt(req.params.tenantId);
      const properties = await tenantTransactionService.getPropertyList(tenantId);
      res.json(properties);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
